...
def prepare_images(style_image: Image, content_image: Image):
...
def train_model(
    ...
    device = 'cpu'
    cnn = models.vgg19(pretrained=True).features.to(device).eval()
    cnn_normalization_mean = cnn_normalization_mean.to(device)
    cnn_normalization_std = cnn_normalization_std.to(device)

    style_img, content_img = prepare_images(style_img, content_img)
